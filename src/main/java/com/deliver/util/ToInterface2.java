...
//      "token": "token"
    ...
    public List<Map<String,Object>> pythonMap(String path) throws JSONException {
        ...
        if (value != null && value != "[]" && value.length() > 2) {
            ...
            JSONArray myJsonObject0 = JSONArray.fromObject(value);
            if (myJsonObject0.size() > 0) {
                for (int i = 0; i < myJsonObject0.size(); i++) {
                    net.sf.json.JSONObject myJsonObject1 = myJsonObject0.getJSONObject(i);  // 遍历 jsonarray 数组，把每一个对象转成 json 对象
